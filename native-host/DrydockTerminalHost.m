...
//     C callback (see DrydockTerminalHostKeyForwardingView.keyCallback below);
...
//     everything the view displays, via drydock_terminal_host_content_view().
@interface DrydockTerminalHostKeyForwardingView : NSView
@property(nonatomic, assign) drydock_terminal_host_key_event_cb keyCallback;
@property(nonatomic, assign) void *keyCallbackUserdata;
@property(nonatomic, assign) drydock_terminal_host_scroll_event_cb scrollCallback;
@property(nonatomic, assign) void *scrollCallbackUserdata;
@property(nonatomic, assign) drydock_terminal_host_mouse_pos_event_cb mousePosCallback;
...
@implementation DrydockTerminalHostKeyForwardingView
...
- (BOOL)isFlipped {
    ...
    // drydock_terminal_host_set_frame's (x, y) can be passed through unmodified
...
- (void)forwardKeyEvent:(NSEvent *)event down:(BOOL)down {
    ...
    // See DrydockTerminalHost.h's Javadoc-style comment on
    // drydock_terminal_host_key_event_cb for why this is needed (Kitty keyboard
...
- (void)updateTrackingAreas {
    ...
    if (self.drydockTrackingArea != nil) {
        [self removeTrackingArea:self.drydockTrackingArea];
    }
    self.drydockTrackingArea = [[NSTrackingArea alloc]
        initWithRect:NSZeroRect
             options:(NSTrackingMouseMoved | NSTrackingActiveAlways | NSTrackingInVisibleRect)
               owner:self
            userInfo:nil];
    [self addTrackingArea:self.drydockTrackingArea];
}
...
- (void)scrollWheel:(NSEvent *)event {
    if (getenv("DRYDOCK_DIAG_SCROLL_LOG") != NULL) {
        NSLog(@"drydock scrollWheel: dy=%f precise=%d cb=%p", event.scrollingDeltaY,
...
#define DRYDOCK_ASSERT_MAIN_THREAD(fn) \
...
drydock_terminal_host_t drydock_terminal_host_create(void *parent_nsview) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_create");
    if (parent_nsview == NULL) {
        NSLog(@"drydock_terminal_host_create: parent_nsview is NULL");
        return NULL;
    }
    NSView *parent = (__bridge NSView *)parent_nsview;
    if (![parent isKindOfClass:[NSView class]]) {
        NSLog(@"drydock_terminal_host_create: parent_nsview is not an NSView");
        return NULL;
    }

    DrydockTerminalHostKeyForwardingView *view =
        [[DrydockTerminalHostKeyForwardingView alloc] initWithFrame:NSZeroRect];
    view.hidden = YES;
    ...
    __weak DrydockTerminalHostKeyForwardingView *weakView = view;
    view.keyMonitor = [NSEvent
        addLocalMonitorForEventsMatchingMask:(NSEventMaskKeyDown | NSEventMaskKeyUp)
                                     handler:^NSEvent *(NSEvent *event) {
        DrydockTerminalHostKeyForwardingView *strongView = weakView;
    ...
    // drydock_terminal_host_destroy. CFBridgingRetain gives us a +1 retain that
    // balances the CFBridgingRelease in destroy.
    return (drydock_terminal_host_t)CFBridgingRetain(view);
}
...
void drydock_terminal_host_set_frame(drydock_terminal_host_t host,
                                  ...
                                  double width,
                                  double height) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_frame");
    if (host == NULL) {
        return;
    }
    ...
    if (!view.hidden && view.superview != nil) {
        // See drydock_terminal_host_set_visible's identical comment: re-assert
...
void *drydock_terminal_host_content_view(drydock_terminal_host_t host) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_content_view");
    ...
    // lifetime is owned by the host and ends at drydock_terminal_host_destroy.
    return host;
}

void drydock_terminal_host_set_visible(drydock_terminal_host_t host, bool visible) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_visible");
    if (host == NULL) {
        return;
    }
    NSView *view = (__bridge NSView *)host;
...
void drydock_terminal_host_set_focused(drydock_terminal_host_t host, bool focused) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_focused");
    if (host == NULL) {
        return;
    }
    NSView *view = (__bridge NSView *)host;
...
void drydock_terminal_host_destroy(drydock_terminal_host_t host) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_destroy");
    if (host == NULL) {
        return;
    }
    ...
    // Balances the CFBridgingRetain in drydock_terminal_host_create.
...
void drydock_terminal_host_set_key_event_callback(drydock_terminal_host_t host,
                                               ...
                                               void *userdata) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_key_event_callback");
    if (host == NULL) {
        return;
    }
    DrydockTerminalHostKeyForwardingView *view = (__bridge DrydockTerminalHostKeyForwardingView *)host;
...
void drydock_terminal_host_set_scroll_event_callback(drydock_terminal_host_t host,
                                                  ...
                                                  void *userdata) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_scroll_event_callback");
    if (host == NULL) {
        return;
    }
    DrydockTerminalHostKeyForwardingView *view = (__bridge DrydockTerminalHostKeyForwardingView *)host;
...
void drydock_terminal_host_set_mouse_pos_event_callback(drydock_terminal_host_t host,
                                                     ...
                                                     void *userdata) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_mouse_pos_event_callback");
    if (host == NULL) {
        return;
    }
    DrydockTerminalHostKeyForwardingView *view = (__bridge DrydockTerminalHostKeyForwardingView *)host;
...
void drydock_terminal_host_set_mouse_button_event_callback(drydock_terminal_host_t host,
                                                        ...
                                                        void *userdata) {
    DRYDOCK_ASSERT_MAIN_THREAD("drydock_terminal_host_set_mouse_button_event_callback");
    if (host == NULL) {
        return;
    }
    DrydockTerminalHostKeyForwardingView *view = (__bridge DrydockTerminalHostKeyForwardingView *)host;
