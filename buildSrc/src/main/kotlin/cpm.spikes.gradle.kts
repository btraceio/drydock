...
registerGateTask(
    ...
    "app.cpm.terminal.ghostty.GhosttySmokeTest"
)
...
registerGateTask(
    ...
    "app.cpm.terminal.Gate0cSpikeLauncher"
) {
    ...
    // driving the window. Pass -Papp.cpm.gate0c.interactive to leave the
    // window open instead.
    if (!providers.gradleProperty("app.cpm.gate0c.interactive").isPresent) {
...
registerGateTask(
    ...
    "app.cpm.terminal.Gate0dSpikeLauncher"
) {
    dependsOn(":buildNativeHost")
    // Scripted checklist by default; -Papp.cpm.gate0d.interactive leaves
    // the window open with a live shell for a human instead.
    if (!providers.gradleProperty("app.cpm.gate0d.interactive").isPresent) {
...
registerGateTask(
    ...
    "app.cpm.terminal.Gate0eSpikeLauncher"
) {
    dependsOn(":buildNativeHost")
    ...
    providers.gradleProperty("app.cpm.gate0e.repo").orNull?.let {
        systemProperty("app.cpm.gate0e.repo", it)
    }
    // Defaults to ~/.local/bin/claude; override with
    // -Papp.cpm.gate0e.claude=<path> if the CLI lives elsewhere.
    providers.gradleProperty("app.cpm.gate0e.claude").orNull?.let {
        systemProperty("app.cpm.gate0e.claude", it)
    }
    // Scripted transcript by default; -Papp.cpm.gate0e.interactive leaves
    // the window open with a live `claude` session instead (for a human to
    // drive the rest of the checklist).
    if (!providers.gradleProperty("app.cpm.gate0e.interactive").isPresent) {
