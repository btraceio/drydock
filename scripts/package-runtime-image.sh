...
#
# Deliverable(s):
#   build/dist/drydock-image-macos-<arch>-<git-short-sha>.tar.gz
#
# Usage:
...
# build/dist/drydock-image-macos-<arch>-<sha>.tar.gz and prints its copy/run
...
package_one() {
    ...
    local archive_name="drydock-image-macos-${arch}-${git_sha}.tar.gz"
    local archive_path="$dist_dir/$archive_name"

    [[ -x "$image_dir/bin/drydock" ]] \
        || fail "$image_dir/bin/drydock not found or not executable; the Gradle task did not produce the expected launcher."
    ...
    echo "  ./$(basename "$image_dir")/bin/drydock"
    echo ""
    echo "The image's default launch target is the real application (app.drydock.Main)."
    echo "To run the Task 5 terminal spike instead (escape hatch for testing):"
    echo "  DRYDOCK_MAIN_CLASS=app.drydock.terminal.Gate0cSpikeLauncher ./$(basename "$image_dir")/bin/drydock"
    echo ""
    if [[ "$kind" == "cross" ]]; then
