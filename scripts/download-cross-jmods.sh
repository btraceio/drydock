...
# buildSrc/src/main/kotlin/drydock/tasks/RuntimeImageTask.kt's assemble()
# checks for exactly this drift before cross-linking and fails clearly if
# they don't match. If you upgrade the project's JDK 26 toolchain to a new
# exact build, update BOTH the URLs/checksums below AND the
# pinnedJmodsBuild constant in RuntimeImageTask.kt together.
#
# Usage:
...
# (buildSrc/src/main/kotlin/drydock/tasks/DownloadCrossJmodsTask.kt) uses the
...
#   DRYDOCK_CROSS_JMODS_ARCHIVE   Use this local .tar.gz path instead of
...
if [[ -n "${DRYDOCK_CROSS_JMODS_ARCHIVE:-}" ]]; then
    [[ -f "$DRYDOCK_CROSS_JMODS_ARCHIVE" ]] || fail "DRYDOCK_CROSS_JMODS_ARCHIVE='$DRYDOCK_CROSS_JMODS_ARCHIVE' does not exist."
    archive_path="$DRYDOCK_CROSS_JMODS_ARCHIVE"
