 ...
 * tab's {@link GhosttyApp} + {@link CpmTerminalHost} pair and its attached
...
final class TerminalBridge {
    ...
    TerminalBridge(GhosttyApp app, CpmTerminalHost host, Region anchor, DoubleSupplier outputScale,
    ...
    void disposeNativeResources() {
        ...
        try {
            ...
            LOG.log(Logger.Level.WARNING, "Failed to close CpmTerminalHost for session " + sessionId.get(), e);
        }
        try {
            app.close();
        } catch (RuntimeException e) {
