...
public final class Gate0eSpike extends Application {
    ...
    private static final boolean AUTO_EXIT = Boolean.getBoolean("app.cpm.gate0e.autoExit");
    private static final String CLAUDE_BIN =
        System.getProperty("app.cpm.gate0e.claude", System.getProperty("user.home") + "/.local/bin/claude");
    private static final String TEST_REPO = System.getProperty("app.cpm.gate0e.repo");
    ...
    public void start(Stage stage) {
        ...
        if (TEST_REPO == null || TEST_REPO.isBlank()) {
            log("FAILED: -Dapp.cpm.gate0e.repo=<throwaway git repo path> is required");
            Platform.exit();
            return;
        }
    ...
    private void buildScript() {
        ...
        steps.add(() -> {
            type("Run `echo $CPM_GATE0E_PROBE` using your bash tool and report the exact output.");
            enter();
            scheduleNext(1500);
        });
