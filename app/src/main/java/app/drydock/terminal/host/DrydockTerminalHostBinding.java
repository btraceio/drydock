...
/**
 * Hand-written FFM bindings for {@code native-host/DrydockTerminalHost.h} (the
 * AppKit host shim, plan section 8).
 *
 * <p>Part of the narrow native boundary (plan section 2.4/4.2): no code
 * outside {@code app.drydock.terminal.host}/{@code app.drydock.terminal.ghostty}
 * may reference {@link MemorySegment}, {@link MethodHandle}, or {@link
 * Linker} for terminal-surface purposes.</p>
 */
final class DrydockTerminalHostBinding {

    private static final Logger LOG = System.getLogger(DrydockTerminalHostBinding.class.getName());

    // The listener shapes are DrydockTerminalHost's own public interfaces
    // (KeyEventListener etc.) -- the binding deliberately defines no
    // duplicates, so DrydockTerminalHost can pass caller listeners through
    // without wrapping.

    /** Native shape of {@code drydock_terminal_host_key_event_cb}. */
    ...
    /** Native shape of {@code drydock_terminal_host_scroll_event_cb}. */
    ...
    /** Native shape of {@code drydock_terminal_host_mouse_pos_event_cb}. */
    ...
    /** Native shape of {@code drydock_terminal_host_mouse_button_event_cb}. */
     ...
     * process-wide singleton -- {@code DrydockTerminalHostLibrary.lookup()}),
     * so the downcall handles are linked once per process instead of once
     * per {@code DrydockTerminalHost}; the handles are stateless and
     * thread-safe to share.
     */
    private static final ConcurrentMap<SymbolLookup, DrydockTerminalHostBinding> INSTANCES = new ConcurrentHashMap<>();

    /** Returns the process-wide binding for {@code lookup}, linking it on first use. */
    static DrydockTerminalHostBinding of(SymbolLookup lookup) {
        return INSTANCES.computeIfAbsent(lookup, DrydockTerminalHostBinding::new);
    }
    ...
    private DrydockTerminalHostBinding(SymbolLookup lookup) {
        ...
        this.create = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_create"),
            FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );

        // void drydock_terminal_host_set_frame(host, double, double, double, double);
        this.setFrame = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_set_frame"),
        ...
        // void* drydock_terminal_host_content_view(host);
        this.contentView = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_content_view"),
            FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );

        // void drydock_terminal_host_set_visible(host, bool);
        this.setVisible = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_set_visible"),
            FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.JAVA_BOOLEAN)
        );

        // void drydock_terminal_host_set_focused(host, bool);
        this.setFocused = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_set_focused"),
            FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.JAVA_BOOLEAN)
        );

        // void drydock_terminal_host_destroy(host);
        this.destroy = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_destroy"),
            FunctionDescriptor.ofVoid(ValueLayout.ADDRESS)
        );

        // void drydock_terminal_host_set_key_event_callback(host, cb, userdata);
        this.setKeyEventCallback = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_set_key_event_callback"),
            FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );

        // void drydock_terminal_host_set_scroll_event_callback(host, cb, userdata);
        this.setScrollEventCallback = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_set_scroll_event_callback"),
            FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );

        // void drydock_terminal_host_set_mouse_pos_event_callback(host, cb, userdata);
        this.setMousePosEventCallback = linker.downcallHandle(
            find(lookup, "drydock_terminal_host_set_mouse_pos_event_callback"),
            FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );
    ...
    private static MemorySegment find(SymbolLookup lookup, String name) {
        return lookup.find(name)
            .orElseThrow(() -> new IllegalStateException(
                "Symbol '" + name + "' not found in libdrydockterminalhost. Re-run "
                    + "scripts/build-native-host.sh and check native-host/DrydockTerminalHost.h."));
    }
    ...
    MemorySegment create(MemorySegment parentNsView) {
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_create", t);
        }
    }
    ...
    void setFrame(MemorySegment host, double x, double y, double width, double height) {
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_frame", t);
        }
    }
    ...
    MemorySegment contentView(MemorySegment host) {
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_content_view", t);
        }
    }
    ...
    void setVisible(MemorySegment host, boolean visible) {
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_visible", t);
        }
    }
    ...
    void setFocused(MemorySegment host, boolean focused) {
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_focused", t);
        }
    }
    ...
    void destroy(MemorySegment host) {
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_destroy", t);
        }
    }
    ...
    void setKeyEventCallback(MemorySegment host, DrydockTerminalHost.KeyEventListener listener, Arena arena) {
        ...
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_key_event_callback", t);
        }
    }
    ...
    void setScrollEventCallback(MemorySegment host, DrydockTerminalHost.ScrollEventListener listener, Arena arena) {
        ...
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_scroll_event_callback", t);
        }
    }
    ...
    void setMousePosEventCallback(MemorySegment host, DrydockTerminalHost.MousePosEventListener listener, Arena arena) {
        ...
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_mouse_pos_event_callback", t);
        }
    }
    ...
    void setMouseButtonEventCallback(MemorySegment host, DrydockTerminalHost.MouseButtonEventListener listener, Arena arena) {
        ...
        try {
            ...
            throw new HostNativeCallException("drydock_terminal_host_set_mouse_button_event_callback", t);
        }
    }
