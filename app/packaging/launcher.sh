#!/bin/bash
# The `build/image/bin/drydock` launcher (plan section 23.2),
# copied verbatim into the jlink runtime image by the `runtimeImage` task
# (see buildSrc/src/main/kotlin/drydock/tasks/RuntimeImageTask.kt). No
...
#   loading (GhosttyNativeLibrary, DrydockTerminalHostLibrary) always uses
...
#   its example.
# - The entry point defaults to app.drydock.Main, the real application. Earlier
...
#   app.drydock.Main a real embedded terminal (managed Claude sessions in
#   tabs). DRYDOCK_MAIN_CLASS=app.drydock.terminal.Gate0cSpikeLauncher (or any
...
#   source set and are NOT bundled in the image; point DRYDOCK_EXTRA_JVM_ARGS
...
# DRYDOCK_MAIN_CLASS / DRYDOCK_EXTRA_JVM_ARGS are internal escape hatches, e.g. to
...
# header comment above for why the default main class is app.drydock.Main (the
# real application).
MAIN_CLASS="${DRYDOCK_MAIN_CLASS:-app.drydock.Main}"
